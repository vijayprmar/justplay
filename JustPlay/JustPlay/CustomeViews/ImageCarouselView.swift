            ...
            ZStack(alignment: .bottom) {
                ...
                HStack(spacing: 3) {
                    ...
                    ForEach(0..<self.numberOfImages, id: \.self) { index in
                         // 4
                        RoundedRectangle(cornerSize: CGSize(width: 25, height: 6))
                            .frame(width: index == self.currentIndex ? 25 : 6,
                                   height: index == self.currentIndex ? 8 : 6)
                            .foregroundColor(index == self.currentIndex ? Color.appRed : .white)
                           // .overlay(RoundedRectangle(cornerSize: CGSize(width: 8, height: 8)).stroke(Color.gray, lineWidth: 1))
                            .padding(.bottom, 8)
                            .animation(.spring())
                    }
                }
